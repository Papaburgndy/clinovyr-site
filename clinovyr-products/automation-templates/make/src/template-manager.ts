...
function walkAndSubstitute(
  ...
  if (Array.isArray(value)) {
    return value.map((item, index) =>
      walkAndSubstitute(item, replacements, `${currentPath}[${index}]`, substitutions)
    );
  }
...
export class TemplateLibrary {
  ...
  constructor(rootDir: string = path.resolve(__dirname, "..")) {
  ...
  loadCatalog(): Manifest {
    ...
    this.manifest = raw as Manifest;
    return this.manifest;
  }

  getTemplate(id: string): TemplateMetadata {
  ...
  listByVertical(vertical: string): TemplateMetadata[] {
    const normalized = vertical.trim().toLowerCase();
    return this.listTemplates().filter(
      (template) =>
        template.vertical.some((entry) => entry.toLowerCase() === "all") ||
        template.vertical.some((entry) => entry.toLowerCase() === normalized)
    );
  }

  loadBlueprint(templateId: string): unknown {
    ...
    return JSON.parse(fs.readFileSync(blueprintPath, "utf-8")) as unknown;
  }

  validateBlueprint(json: unknown): BlueprintValidationResult {
    const errors: string[] = [];
  ...
  private ensureManifest(): Manifest {
    if (!this.manifest) {
  ...
  private validateManifestStructure(raw: unknown): string[] {
    const errors: string[] = [];
    ...
    raw.templates.forEach((entry, index) => {
      ...
      const requiredStringFields: Array<keyof TemplateMetadata> = [
        "id",
        "name",
        "description",
        "estimatedROI",
      ...
      if (
        typeof entry.setupTimeMinutes !== "number" ||
  ...
  private buildReplacementMap(clientConfig: ClientConfig): Map<string, string> {
